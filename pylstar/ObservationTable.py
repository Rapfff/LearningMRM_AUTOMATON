...
class ObservationTable(object):
    ...
    def add_counterexample(self, input_word, output_word):
        ...
        if len(input_word) != len(output_word):
            raise Exception("Output word must have the same length as input word")
    ...
    def __get_row(self, row_name):
        """This method returns the specified row (output word)
...
"""
    ...
    def build_hypothesis(self):
        """This method returns an Automaton that follows the observation table.

        If the observation table is closed and consistent, it is possible to construct
        an hypothesis automaton. Each state of the automaton maps to a row of S in the observation table.
